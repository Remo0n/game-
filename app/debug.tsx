...
export default function DebugScreen() {
  return __DEV__ ? <Inspector /> : <Redirect href="/home" />;
}
function Inspector() {
  ...
  function run() {
    ...
    setReport(
      [
        ...
        showSolution
          ? `solution pieces ${level.solution.placements.length}`
          : "solution hidden",
      ].join("\n"),
    );
  ...
  return (
    <Screen>
      ...
      <ScrollView horizontal contentContainerStyle={styles.pills}>
        {DIFFICULTIES.map((item) => (
          <Text
            ...
            style={[styles.pill, difficulty === item && styles.pillOn]}
          >
            {item}
          </Text>
        ))}
      ...
      <ScrollView horizontal contentContainerStyle={styles.pills}>
        {VARIATIONS.map((item) => (
          <Text
            ...
            style={[styles.pill, variation === item && styles.pillOn]}
          >
            {item}
          </Text>
        ))}
      ...
      {preview ? (
        <MiniGrid
          levelTarget={preview.level.targetShape}
          solution={showSolution ? preview.level.solution : null}
        />
      ) : null}
...
function MiniGrid({
  ...
  solution: {
    ...
    pieces: { id: string; shape: { x: number; y: number }[] }[];
  } | null;
}) {
    ...
    >
      {[...keys].map((key) => {
        ...
        return (
          <View
            ...
            style={[
              styles.mini,
              { left: (x - bounds.minX) * cell, top: (y - bounds.minY) * cell },
            ]}
          />
        );
      })}
      {solution ? (
        <Text style={styles.note}>
          Solution overlay is on for this inspector only.
        </Text>
      ) : null}
    </View>
  );
