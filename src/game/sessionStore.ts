...
import type { CutOrientation, LaserLine, PlayableLevel, ToolKit } from "../engine/types";
...
function finishIfSolved(attempt: AttemptState, level: PlayableLevel, startedAt: number): RunResult | null {
...
export const useSession = create<SessionState>((set, get) => ({
  ...
  cut: (pieceId, orientation, position) => {
    ...
    if (!attempt || !level || get().result) return false;
    const piece = attempt.pieces.find((item) => item.id === pieceId);
    const snapped = piece ? nearestAxisCut(piece.shape, orientation, position) : null;
    if (!snapped) {
      set({ attempt: { ...attempt, wastedMoves: attempt.wastedMoves + 1 } });
      return false;
    }
    const next = commitCut(attempt, pieceId, snapped.orientation, snapped.position);
  ...
  laser: (pieceId, line) => {
    ...
    set({ attempt: next, result: finishIfSolved(next, level, startedAt) });
    return next.cutsUsed > attempt.cutsUsed;
  },
  lineSplit: (pieceId) => {
    const { attempt, level, startedAt } = get();
    ...
    const next = commitLineSplit(attempt, pieceId, split.orientation, split.position);
    set({ attempt: next, result: finishIfSolved(next, level, startedAt), cutMode: next.cutsUsed < next.allowedCuts });
  ...
  drop: (pieceId, x, y) => {
    ...
    if (!attempt || !level) return false;
    const piece = attempt.pieces.find((item) => item.id === pieceId);
    const snap = piece ? findSnap(piece.shape, attempt.target, occupiedKeys(attempt, pieceId), x, y) : null;
    if (!snap) {
      set({ attempt: { ...attempt, wastedMoves: attempt.wastedMoves + 1 } });
      return false;
    }
    const dropped = placePiece(attempt, pieceId, snap.x, snap.y);
    set({ attempt: dropped.state, result: finishIfSolved(dropped.state, level, startedAt) });
    return dropped.placed;
  },
  rotate: (pieceId) => {
    const { attempt } = get();
    if (!attempt) return;
    set({ attempt: rotatePiece(attempt, pieceId) });
  },
  extraCut: () => {
    const { attempt } = get();
    if (!attempt) return;
    set({ attempt: useExtraCut(attempt), cutMode: true });
  },
  hint: () => {
  ...
  undoMove: () => {
    const { attempt } = get();
    if (!attempt) return;
    set({ attempt: undo(attempt), result: null });
  },
  reset: () => {
    const { attempt } = get();
    if (!attempt) return;
    set({ attempt: resetAttempt(attempt), result: null, timedOut: false, startedAt: Date.now(), cutMode: attempt.initial.allowedCuts > 0 });
  },
  expire: () => {
    const { attempt } = get();
    if (!attempt || get().result) return;
    set({ attempt: resetAttempt(attempt), timedOut: true, startedAt: Date.now(), result: null });
  },
  boost: (tool) => {
    const { attempt } = get();
    if (!attempt || attempt.kit[tool] >= 6) return;
    set({ attempt: { ...attempt, kit: { ...attempt.kit, [tool]: attempt.kit[tool] + 1 } } });
  },
}));
